...
> DESIGN.md は 2026-07-23 に v0.2 へ改訂。v0.1 の実装が「平面的・単調」だったため、**金の追加 / 英字併走 / 面構成 / スケール拡大**を導入した。視覚仕様の正本は DESIGN.md、その根拠となるモックは `docs/design/`（`modern/{home,craft-detail,events}.html` + `assets/{common,patterns}.css`）。
...
- **`docs/design/` のモックはデスクトップ専用（`min-width:1200px`）で SP 未設計**。モバイルは DESIGN §8 の表に従って実装側で設計する。**モック内の画像は外部ホストの仮素材なので実装に持ち込まない**
